\LevelZero{Transformer}\label{ch:transformer}
\LevelOne{Command Transformer}
The following commands can be transformed:
\begin{enumerate}
    \item /blockcycling = /give \player minecraft:debug\_stick
    \item /blockinfo = use the command \cmd{/info block} provided by \ttt{carpet} mod.
    ...
    \item /item\_editor = /give with command generator, or use some client-side mod to edit items with commands.
...
\begin{tips}{Use command bundle module to create transformed commands}
    See~\nameref{ch:command_bundle}
\end{tips}

\clearpage
\LevelOne{Command Generator}
