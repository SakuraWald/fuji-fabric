...
\begin{note}{Delay command is only used for short term job}
    The command \cmd{/delay} is only used to perform short-term job, and will not be persisted on server shutdown.
...
\begin{example}{A basic usage}
    \cmd{/delay 3 say three seconds passed}
\end{example}

\begin{example}{A nested delay}
    \cmd{/delay 1 delay 2 delay 3 say 6 seconds passed.}
\end{example}
