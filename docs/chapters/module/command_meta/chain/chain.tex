\LevelThree{chain}\label{ch:chain}

\LevelFour{Purpose}
A chain command allows you to run another 2 commands, the first is any command, and the second is the chain command.

\begin{note}{The return value of a command}
    In vanilla minecraft, the return value of command, is an integer:
    \begin{description}
        \item[<0]{failed}
        \item[=0] {passed}
        \item[>0]{success}
    \end{description}
\end{note}

\LevelFour{Command}
\fcmd{/chain}


\LevelFour{Example}
\begin{example}{A nested chain.}
    \cmd{/chain say 1 chain say 2 chain say 3}
\end{example}

\begin{example}{A breakable chain.}
    \cmd{/chain bad command here chain say 2}
\end{example}

\begin{example}{Use chain command with predicate command}
    \cmd{/run as player \player chain test-level-perm \ph{player:name} 4 chain say value is true}
\end{example}
